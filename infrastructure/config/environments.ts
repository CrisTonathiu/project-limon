 ...
 * each gets its own VPC, database, bucket, user pool, queues, secrets and ECS services.
...
export type EnvConfig = {
  ...
  prefix: string;
  natGateways: number;
  /** Aurora Serverless v2 cluster, or a single provisioned RDS PostgreSQL instance (cheaper for dev). */
  database:
    | { kind: 'aurora-serverless'; minAcu: number; maxAcu: number; readers: number; deletionProtection: boolean }
    | { kind: 'rds-instance'; instanceType: string; allocatedStorageGiB: number; deletionProtection: boolean };
...
export const environments: Record<EnvName, EnvConfig> = {
  development: {
    ...
    database: { kind: 'rds-instance', instanceType: 't4g.micro', allocatedStorageGiB: 20, deletionProtection: false },
  ...
  staging: {
    ...
    database: { kind: 'aurora-serverless', minAcu: 0.5, maxAcu: 4, readers: 0, deletionProtection: true },
  ...
  production: {
    ...
    database: { kind: 'aurora-serverless', minAcu: 2, maxAcu: 32, readers: 1, deletionProtection: true },
